...
import { IconDownload, IconRefresh, IconArrowLeft, IconPlayerPlay } from "@tabler/icons-react";
...
export default function RunHistory() {
  ...
  const download = async (run: ReportRun) => {
    ...
    if (!res.ok) {
      notifications.show({ color: "red", message: "Archivo no disponible" });
      return;
    }
    ...
    a.href = url;
    a.download = run.file_path?.split(/[\\/]/).pop() || `reporte_${run.id}`;
    a.click();
  ...
  const runNow = async () => {
    try {
      ...
      notifications.show({ color: "blue", message: "Ejecución encolada" });
      setTimeout(load, 1500);
    } catch (e: any) {
      notifications.show({ color: "red", message: apiError(e, "No se pudo ejecutar") });
    }
  };
    ...
    <Stack maw={1000} mx="auto">
      <Group justify="space-between">
        <Group>
          ...
          <div>
            <Title order={2}>Historial de corridas</Title>
        ...
        <Group>
          ...
          {canRun && (
            <Button leftSection={<IconPlayerPlay size={16} />} color="green" onClick={runNow}>
              Ejecutar ahora
            </Button>
          )}
        </Group>
            ...
            <Table.Tbody>
              {runs.map((run) => (
                <Table.Tr key={run.id}>
                  ...
                  <Table.Td>{run.trigger === "scheduled" ? "Programado" : "Manual"}</Table.Td>
                  ...
                  <Table.Td>
                    {run.delivery_status ? (
                      <Badge
                        variant="dot"
                        color={run.delivery_status === "failed" ? "red" : "teal"}
                      >
                        {run.delivery_status}
                      </Badge>
                    ) : (
                  ...
                  <Table.Td ta="right">
                    {run.file_path ? (
                      <Tooltip label="Descargar">
                        <ActionIcon variant="subtle" onClick={() => download(run)}>
